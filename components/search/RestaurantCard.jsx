...
const RestaurantCard = ({ id, onCardClick, data }) => {
  ...
  return (
    <li
      className="flex flex-col snap-start rounded-xl md:h-[150px] md:max-w-full cursor-pointer md:flex-row md:overflow-hidden md:pr-2 hover:md:bg-[#f4f5f4] md:shadow-md"
      id={id}
    ...
    >
      ...
      <div className="md:w-[calc(100%-104px)] mt-1 md:flex-1 md:pl-2 lg:w-[calc(100%-164px)]">
        ...
        {/* <div className=" mt-1 flex items-center justify-start">
          <p className="font-medium text-black">{dollar(data.price_level)}</p>
        </div> */}
        ...
        <div className=" mt-1 flex items-center justify-start">
          <p>
            {data.opening_hours.open_now ? <span className="text-brand-700">營業中</span> : <span className="text-sunsetOrange">打烊中</span>} · {place[place.length - 1]}
          </p>
        </div>
      </div>
    </li>
